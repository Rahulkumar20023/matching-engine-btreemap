...
\- \*\*Best bid/ask\*\*: `O(log n)` — `.iter().next\_back()` / `.iter().next()`
...
> \*\*Measurement\*\*: Per-operation `Instant::now()` with `std::hint::black\_box`  
...
| passive\_add | 278ns | 200ns | 400ns | 900ns | 3,000ns | 15,638µs |

| mixed\_workload | 253ns | 100ns | 300ns | 800ns | 3,800ns | 2,124µs |

| sweep\_worst\_case | 2,524ns | 2,400ns | 2,600ns | 4,300ns | 21,000ns | 155µs |
...
&#x20;   │   ├── book\_node.rs         — book node definition

&#x20;   │   └── price\_level.rs       — price level helpers
